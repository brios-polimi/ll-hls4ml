...
def _tensor_content_manifest(
    config: dict,
    config_dir: Path,
    tensor_dir: Path,
    vocab_path: Path,
    dataset: HeteroGraphDataset,
    exemplar_ds: HeteroGraphDataset,
    *,
    main_process: bool,
    distributed: bool,
) -> dict[str, object]:
    expected_paths = [
        *dataset.paths,
        *dataset.duplicate_paths,
        *exemplar_ds.paths,
        *exemplar_ds.duplicate_paths,
    ]
    expected_paths.extend(
        path
        for path in (tensor_dir / "labels.json", vocab_path)
        if (
            path.is_file()
            and path.resolve().is_relative_to(tensor_dir.resolve())
        )
    )

    manifest = None
    if main_process:
        configured_path = config.get("tensor_manifest_path")
        if configured_path:
            manifest_path = _config_path(configured_path, config_dir)
            manifest = load_content_manifest(manifest_path)
            assert_manifest_covers(manifest, expected_paths, tensor_dir)
        else:
            manifest = build_content_manifest(expected_paths, tensor_dir)

    payload = [manifest]
    if distributed:
        dist.broadcast_object_list(payload, src=0)
    if payload[0] is None:
        raise RuntimeError("Tensor content manifest was not constructed")
    return payload[0]
...
- Tensor content snapshot (SHA-256): `{resolved_config["tensor_snapshot_id"]}`
- Tensor source revision: `{resolved_config.get("tensor_source_revision", "not recorded")}`
...
def main() -> None:
    ...
    tensor_manifest = _tensor_content_manifest(
        config,
        config_dir,
        tensor_dir,
        vocab_path,
    ...
    resolved_config = {
        **config,
        ...
        "seed": seed,
        "tensor_snapshot_id": tensor_manifest["snapshot_sha256"],
        "tensor_manifest_file_count": len(tensor_manifest["files"]),
        "result_tensor_manifest_path": str(
            run_dir / "tensor_manifest.json"
        ),
